...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ex6-6-12) (read-case-sensitive #t) (teachpacks ((lib "draw.rkt" "teachpack" "htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "draw.rkt" "teachpack" "htdp")) #f)))
;; 6.6.7
...
;; 6.6.10
;; Let's use fun-for-rect to develop translate-rectangle

;; Here's that template again for ez ref
;; fun-for-rect : rectangle -> ???
;(define (fun-for-rect a-rectangle)
;  ... (rectangle-ul a-rectangle) ...
;  ... (rectangle-w a-rectangle) ...
...
;; to create a new rectangle that is the old rectangle,
;; except with its upper left corner shift the specified number of pixels to the right

;; example
;; if rectangle-ul a-rectangle is 50, 100
;; then (translate-rectangle a-rectangle 35)
;; should produce a rectangle with rectangle-ul 85, 100

;; let's go for it
(define (translate-rectangle a-rect delta)
  (make-rectangle
   (make-posn
    (+ delta (posn-x (rectangle-ul a-rect)))
    (posn-y (rectangle-ul a-rect)))
   (rectangle-w a-rect)
   (rectangle-h a-rect)
   (rectangle-c a-rect)))

;; now let's test!
(define rectangle-10 (make-rectangle (make-posn 50 100) 10 10 'blue))
(= 85
   (posn-x (rectangle-ul (translate-rectangle rectangle-10 35))))
...
(stop)
(start 300 300)
(define example-rect-11 (make-rectangle (make-posn 100 100) 100 50 'black))
(draw-a-rectangle example-rect-11)
(clear-a-rectangle example-rect-11)
...
(stop)
(start 300 300)
(define example-rect-12 (make-rectangle (make-posn 100 100) 100 50 'black))
(draw-and-clear-rectangle example-rect-12)

;; move-rectangle : number rectangle -> rectangle
;; to draw and clear a rectangle and translate it by number pixels to the right
(define (move-rectangle delta a-rect)
  (cond
    [(draw-and-clear-rectangle a-rect) (translate-rectangle a-rect delta)]
    [else a-rect]))

(stop)
(start 300 100)
(define example-rect-122 (make-rectangle (make-posn 25 25) 75 50 'orange))
(draw-a-rectangle
 (move-rectangle 25
                (move-rectangle 25
                                (move-rectangle 25
                                                (move-rectangle 25 example-rect-122)))))